...
namespace PlainElastic.Net.Tests.Buildres.Queries
{
    ...
    class When_TermQuery_with_custom_part_built
    {
        ...
        It should_contain_custom_part = () => result.ShouldContain(@"'boost': 5".SmartQuote());

        It should_return_correct_query = () => result.ShouldEqual(@"{ 'term': { 'Property1': { 'value': 'one','boost': 5 } } }".SmartQuote());
